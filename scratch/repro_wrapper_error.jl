using Frankenstein
using DifferentialEquations
using ADTypes

println("--- Reproducing FunctionWrapper Error ---")

# Tiny system that should trigger Symbolics
function lorenz!(du, u, p, t)
    du[1] = 10.0 * (u[2] - u[1])
    du[2] = u[1] * (28.0 - u[3]) - u[2]
    du[3] = u[1] * u[2] - (8/3) * u[3]
end

u0 = [1.0, 0.0, 0.0]
tspan = (0.0, 1.0)
prob = ODEProblem(lorenz!, u0, tspan)

println("System size: ", length(u0))

# Force AutoSymbolics to be used
# We can do this by setting ad_available
solver = Frankenstein.FrankensteinSolver()
try
    sol = Frankenstein.solve(prob, solver, ad_available=[AutoSymbolics()])
    println("Solve successful!")
catch e
    println("CAUGHT ERROR:")
    # Print the full error and stack trace
    showerror(stdout, e)
    println()
end
